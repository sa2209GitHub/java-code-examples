...
class Example100 {
  ...
  void bubbleSort(int array[]) {
    ...
    System.out.println("1 for Accending \n2 for Dedcending");
    int sortOrder = input.nextInt();

    // run loops two times
    // first loop access each element of the array
    for (int i = 0; i < size - 1; i++)
      // second loop performs the comparsion in each iteration
      for (int j = 0; j < size - i - 1; j++)
        ...
        if (sortOrder == 1) {
          ...
          if (array[j] > array[j + 1]) {
            // swap if left element is greather than right
            int temp = array[j];
            array[j] = array[j + 1];
            array[j + 1] = temp;
          }
        ...
        // sort the array in descending order
          ...
          // compress the in descending order
          if (array[j] < array[j + 1]) {
